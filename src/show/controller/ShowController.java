...
public class ShowController implements ShowRepository {
	...
	public boolean dataNascimento(String dataN) {
		...
		if ((dataNascFormat != null) && (curDate != null)) {
	...
	public void atualizarCadastro(Cadastro cadastroNovo, Cadastro cadastroAtual) {
		for (Cadastro cadastro : cadastrarUsuario) {
			if(cadastro == cadastroAtual) {
				cadastrarUsuario.set(cadastrarUsuario.indexOf(cadastroAtual), cadastroNovo);
			}
		}
	}
	...
	public boolean agendarCamarote(String nomeAgenda, int pacote) {
			...
			if(reservado(numCam, pacote) == false) {
